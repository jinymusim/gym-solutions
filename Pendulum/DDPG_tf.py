...
class Trainer:
    ...
    def evaluate(self, j):
        ...
        for _ in range(self.args.evaluate_for):
            ...
            total_reward = 0
            while not done:
                action = self.agent.actor_forward(np.asarray(state).reshape(1, -1))[0]
    ...
    def train(self):
        ...
        while True:
            ...
            while not done:
                action = np.clip(self.agent.actor_forward(np.asarray(state).reshape(1, -1))[0] * np.random.normal(scale =0.1), self.env.action_space.low, self.env.action_space.high)
                next_state, reward, terminated, truncated, _ = self.env.step(action)
                done = terminated or truncated
                ...
                if len(replay_buffer) >= self.args.min_buffer:
                    ...
                    returns = rewards[:,None] + self.args.gamma * self.agent.critic_forward(np.asarray(next_states).reshape(self.args.batch_size, -1))[0] * np.logical_not(dones)[:,None]

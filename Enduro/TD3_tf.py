...
parser.add_argument("--num_filters", default=[4,4], type=list, help="Number of filters for convolution")
...
parser.add_argument("--num_pixels", default=48, type=int, help="number of frames to stack")
...
class TD3:
    ...
    def train(self, states, actions, returns):
        ...
        with tf.GradientTape() as other_critic_tape:
            predict_other_critic = self.other_critic(states, actions )
    ...
    def critic_forward(self, states):
        actor_actions = self.target_actor(states) + np.random.normal(scale =0.1)
        actor_actions = actor_actions - tf.reduce_min(actor_actions, axis=-1, keepdims=True)
        actor_actions = actor_actions / tf.reduce_sum(actor_actions, axis=-1, keepdims=True)
        
        return tf.minimum(self.target_critic(states, actor_actions), self.other_target_critic(states, actor_actions))
...
class Trainer:
    ...
    def train(self):
        ...
        while True:
            ...
            while not done:
                action = self.agent.actor_forward(np.asarray(state, dtype=np.float32).reshape(1, *self.env.observation_space.shape)).numpy()[0] + np.random.normal(scale =0.1) 

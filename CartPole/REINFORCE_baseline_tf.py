...
class Agent:
    ...
    def __init__(self, env: gym.Env, args: argparse.Namespace) -> None:
        ...
        baseline_inputs = tf.keras.layers.Input(self.observation_space)
        baseline_hidden = tf.keras.layers.Dense(args.hidden_layer, activation="relu")(baseline_inputs)
        baseline_outputs = tf.keras.layers.Dense(1, activation=None)(baseline_hidden)
        
        self.baseline_model = tf.keras.Model(inputs=baseline_inputs, outputs=baseline_outputs)
        self.baseline_model.compile(
            optimizer=tf.keras.optimizers.Adam(learning_rate=args.lr),
    ...
    def train(self, states, actions, returns):
        ...
        with tf.GradientTape() as baseline_tape:
            baseline_outputs = self.baseline_model(states)
            baseline_loss = self.baseline_model.compiled_loss(returns, baseline_outputs)
        baseline_grad = baseline_tape.gradient(baseline_loss, self.baseline_model.trainable_variables)
        
        with tf.GradientTape() as tape:
            outputs = self.model(states)
            loss = self.model.compiled_loss(actions, outputs, returns - tf.squeeze(baseline_outputs) )
        grad = tape.gradient(loss, self.model.trainable_variables)
        
        self.model.optimizer.apply_gradients(zip(grad, self.model.trainable_variables))
        self.baseline_model.optimizer.apply_gradients(zip(baseline_grad, self.baseline_model.trainable_variables))

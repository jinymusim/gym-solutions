...
class A3C:
    ...
    class Actor(tf.keras.Model):
        ...
        def call(self, inputs: tf.Tensor):
            ...
            action_prob_scaled = tfp.bijectors.Scale((env.action_space.high - env.action_space.low) / 2)(action_prob)
            action_prob_scaled = tfp.bijectors.Shift((env.action_space.high + env.action_space.low) / 2)(action_prob_scaled)
    ...
    def train(self, states, actions, returns):
        ...
        with tf.GradientTape() as critic_tape:          
            ...
            critic_loss = self._critic.loss(returns, predict_critic)
...
class Trainer:
    ...
    def evaluate(self, j):
        ...
        for _ in range(self.evaluate_for):
            ...
            total_reward = 0
            while not done:
                action, _, _ = self.agent.actor_forward(np.asarray(state).reshape(1, -1))[0]
    ...
    def train(self):
        ...
        while True:
            for _ in range(self.args.evaluate_each if self.args.evaluate_each != None else 50):
                ...
                action = self.agent.actor_forward(state)
                ...
                if len(replay_buffer) >= self.args.min_buffer:
                    ...
                    returns = rewards[:,None] + self.args.gamma * self.agent.critic_forward(next_states) * np.logical_not(dones)[:,None]

                    self.agent.train(states, actions, returns)  
